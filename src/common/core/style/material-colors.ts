...
export class MaterialColors {
    ...
    public get colors() {
        if (!this._colors) {
            ...
            Object.keys(this.palet).forEach((baseColorName) => {
                let baseColor = MaterialColor.fromHex(this.palet[baseColorName]['500']) as MaterialColor;
                baseColor.subColors = [];
                baseColor.name = baseColorName;
                this._colors.push(baseColor);
                Object.keys(this.palet[baseColorName]).forEach((subColorName) => {
                    let subColor = MaterialColor.fromHex(this.palet[baseColorName][subColorName]) as MaterialColor;
    ...
    public getPalet(subColor: string) {
        if (!this.palettes) {
            this.palettes = {};
            Object.keys(this.palet).forEach((baseColorName) => {
                let baseColorObj = this.palet[baseColorName];
