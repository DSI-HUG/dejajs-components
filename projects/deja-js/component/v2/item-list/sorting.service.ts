...
export class SortingService {
     ...
     * @return Liste triée.
     */
    public sort(list: unknown[], sortInfo: SortInfos | SortInfos[]): unknown[] {
        if (list?.length) {
     ...
     */
    public sortTree(tree: unknown[], sortInfo: SortInfos | SortInfos[], childrenField = 'items'): unknown[] {
        return this.sort(tree, sortInfo).map(child => {
            const indexed = child as Record<string, unknown>;
            if (indexed?.[childrenField] && indexed?.[childrenField] instanceof Array) {
                this.sortTree(indexed[childrenField] as unknown[], sortInfo, childrenField);
            }
            return indexed;
        });
    }
